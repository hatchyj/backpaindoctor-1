---
draft: false
order: 5
title: "Tennis & Golfer's Elbow"
description: "Non-surgical treatment of tennis elbow (lateral epicondylitis) and golfer's elbow in Brisbane with musculoskeletal GP Dr Joshua Hatch."
intro: Tennis elbow and golfer's elbow are common tendinopathies affecting the outer and inner elbow respectively. They are usually overuse-related and typically improve with the right approach to load and rehabilitation.
symptoms:
  - Pain over the outer elbow (tennis elbow) or inner elbow (golfer's elbow)
  - Pain with gripping, lifting or twisting movements
  - Tenderness over the affected tendon attachment
  - Weakness of grip
treatmentLinks:
  - name: Focused Shockwave (ESWT)
    href: /non-surgical-treatments/shockwave-therapy-eswt
  - name: Platelet Rich Plasma (PRP)
    href: /non-surgical-treatments/platelet-rich-plasma-treatment
  - name: Prolotherapy
    href: /non-surgical-treatments/prolotherapy-treatment
faqs:
  - question: How long does tennis elbow last?
    answer: Many cases improve over several months, though it can be persistent. A program that gradually loads the tendon is central to recovery.
  - question: Should I rest it completely?
    answer: Complete rest is usually not the answer. Modifying aggravating activities while gradually loading the tendon tends to work better than avoidance alone.
  - question: What if it does not settle?
    answer: For persistent cases, treatments such as shockwave therapy or PRP have been studied for elbow tendinopathy. We will discuss whether these are reasonable options for you and what the evidence shows.
---

Tennis elbow (lateral epicondylitis) and golfer's elbow (medial epicondylitis) are tendinopathies of the muscles that attach around the elbow. Despite the names, they are usually related to repetitive load rather than to playing a particular sport.

The most important treatment is a graded exercise program that progressively loads the affected tendon, alongside modifying the activities that aggravate it. This is well-evidenced and effective for many people, though it requires patience as tendons respond slowly.

For cases that do not settle with first-line measures, additional options such as shockwave therapy and PRP have been studied for elbow tendinopathy. We assess your elbow, confirm the diagnosis, and give you an honest view of which options are worth considering and which are not.
