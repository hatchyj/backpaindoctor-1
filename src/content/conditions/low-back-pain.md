---
draft: false
order: 1
title: "Low Back Pain"
description: "Assessment and non-surgical management of low back pain in Brisbane with musculoskeletal GP Dr Joshua Hatch."
intro: Low back pain is one of the most common reasons people seek musculoskeletal care. For most people it is not caused by a serious problem and improves with the right approach to movement, load and pain management.
symptoms:
  - Pain across the lower back, sometimes radiating to the buttock or thigh
  - Stiffness, especially after rest or in the morning
  - Pain aggravated by certain postures, bending or lifting
  - Difficulty with prolonged sitting or standing
treatmentLinks:
  - name: Trigger Point Therapy
    href: /non-surgical-treatments/trigger-point-therapy-treatment
  - name: Prolotherapy
    href: /non-surgical-treatments/prolotherapy-treatment
  - name: EMTT
    href: /non-surgical-treatments/emtt-magnetotransduction
faqs:
  - question: Do I need an MRI for my back pain?
    answer: Often not. For most acute low back pain without warning features, imaging does not change early management and can sometimes lead to unnecessary worry or treatment. We assess whether imaging is genuinely needed based on your history and examination.
  - question: When should I be concerned about back pain?
    answer: Features such as significant trauma, unexplained weight loss, fever, progressive leg weakness, or problems with bladder or bowel control warrant prompt assessment. If you have these, seek medical care without delay.
  - question: Will I need surgery?
    answer: Most low back pain is managed without surgery. Surgery is reserved for specific situations and is not the first option for the majority of people.
---

Low back pain is extremely common and, in most cases, is not a sign of serious disease. The lower back is a complex structure of joints, discs, ligaments and muscles, and pain can arise from several of these without there being a single, clearly identifiable "cause" on a scan.

The most important early steps for most people are staying active within reasonable limits, avoiding prolonged bed rest, and using simple measures to manage pain while the back settles. Physiotherapy and a graded return to normal activity are central to recovery.

Our role is to assess your back carefully, identify any features that need further investigation, and give you an honest plan. Where pain is persistent and has not responded to first-line measures, we can discuss whether additional treatments are reasonable in your situation — always alongside, not instead of, good rehabilitation.
