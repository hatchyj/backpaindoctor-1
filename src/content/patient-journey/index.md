---
title: "Patient Journey"
description: "meta description"
draft: false

#intro
intro:
  title: A tailored approach to get you free from pain.
  subtitle: The patient journey
  description: Nearly all pain and injuries can be treated without surgery, making regenerative treatments a promising option. If you’ve tried physiotherapy or other injections therapies like cortisone, and still haven’t seen the desired results, regenerative treatments may provide the relief you need. Even if you’ve been advised to undergo surgery, there’s a good chance that non-surgical options can help you avoid it. While surgery is sometimes necessary, it should not be the first option.

#service
service:
  - title: History, Examination, Diagnosis
    subtitle: First consultation
    description: "During your initial consultation with our <a href='/about'>musculoskeletal doctor</a>, you'll undergo a comprehensive evaluation of your medical history and physical symptoms. This includes a discussion of your symptoms, when they began, and any factors that may exacerbate or alleviate them. The doctor may also inquire about your previous medical history, medication use, and surgical history.<br/><br/>Afterwards, the doctor will perform a physical examination to evaluate your range of motion, muscle strength, and identify any areas of tenderness or swelling. They may order imaging tests like X-rays or MRI scans to assist with the diagnosis."
    image: /images/back-pain-history-examination-diagnosis.jpg
    image_style: rounded-2xl shadow-xl max-w-[70%]
  - title: Diagnosis, Discussion and Treatment plan
    subtitle: First consultation
    description: "Using the information obtained from the examination, the doctor will develop a diagnosis and discuss potential treatment options. Treatment options can range from medications, manual therapy, injections, or referral for surgery depending on the severity of the condition.<br/><br/>To achieve an accurate diagnosis and personalised treatment plan, it is important to be open and honest with the doctor during the consultation."
    image: /images/diagnosis-discussion-treatment-plan.jpg
  - title: Therapy Begins
    subtitle: Second Consultation
    description: Following a comprehensive initial consultation, your musculoskeletal doctor will initiate a tailored therapy plan. This plan may encompass a variety of therapeutic modalities or may focus on a specific approach.
    hasCTA: true
    cta:
      href: /treatments
      label: Explore Therapy Teatments
    image: /images/treatment-begins.jpg
  - title: Therapy continues
    subtitle: Follow up
    description: The duration of the treatment phase is flexible and may necessitate multiple rounds of therapy spanning several months. In certain cases, ongoing treatment may be required, albeit with less frequent appointments.
    hasCTA: false
    image: /images/treatment-continues.jpg
  - title: Support and monitor
    subtitle: Pain-free living
    description: Our goal is to help you achieve relief, and once we do, we can gradually reduce the frequency of treatments and provide support as needed to ensure that you maintain your progress.
    hasCTA: true
    cta:
      href: /book-appointment
      label: Book an appointment today
    image: /images/support-and-monitor.jpg
---
